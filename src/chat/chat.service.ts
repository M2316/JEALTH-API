...
규칙:
1. 제공된 Exercise 후보 목록 중에서만 exerciseId를 선택하라.
2. 세트 수, reps, weight는 사용자 메시지에 명시된 숫자를 그대로 사용하라. 추측 금지.
3. weightUnit은 사용자가 'lbs'를 명시하지 않으면 'kg'.
4. reply 필드에는 사용자가 확인할 수 있게 "X 종목 Y세트 이렇게 맞나요?" 형식으로 간결히.
5. 운동 외 질문이 들어오면 draft.exercises를 빈 배열로 두고 reply에 "운동 기록만 도와드려요"라고 답하라.
6. 숫자·단위 해석이 모호하면 confidence를 'low'로 설정하라.
7. 후보 목록이 비어있거나 매칭이 어렵다면 draft.exercises를 빈 배열로 두고 confidence를 'low'로 설정하라.`;
...
export class ChatService {
  ...
  async processMessage(req: ChatRequestDto): Promise<ChatResponseDto> {
    ...
    if (!draft) {
      throw new ServiceUnavailableException('AI 응답을 처리하지 못했습니다');
    }

    const filteredExercises = draft.draft.exercises.filter(
      (e) => e.exerciseId !== SENTINEL,
    );
